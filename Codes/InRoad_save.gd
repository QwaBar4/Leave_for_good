...
func _on_new_pressed() -> void:
	...
	var num := 0
	var scene = 3#МЕНЯТЬ ОБЯЗАТЕЛЬНО ТАК КАК ЭТО ВЛИЯЕТ НА СОХРАНЕНИЕ СЦЕНЫ
	var save_path := "Путь в универ" + SAVE_ENDING
	var phrase_num = $PhraseNum2.text
	while FileAccess.file_exists(SAVES_FOLDER + save_path):
		num += 1
		save_path = "Путь в универ" + str(num) + SAVE_ENDING
